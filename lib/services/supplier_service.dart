...
class SupplierService {
  ...
  Future<PaginatedResponse> fetchSuppliers({
    ...
    try {
      PostgrestTransformBuilder query = supabase
        .from('fornecedor')
        .select();
      if (searchQuery != null && searchQuery.isNotEmpty) {
        query = supabase
            .from('fornecedor')
            .select('*')
            .filter('frn_nome', 'ilike', '%$searchQuery%')
            .filter('frn_cnpj', 'ilike', '%$searchQuery%');
      }
      if (sortParams.activeSortColumnDataField != null) {
        query = query.order(sortParams.activeSortColumnDataField!, ascending: sortParams.isAscending);
      }
      ...
      var countQuery = supabase
        ...
        .select('id');
      if (searchQuery != null && searchQuery.isNotEmpty) {
        countQuery = countQuery.or('frn_nome.ilike.%$searchQuery%,frn_cnpj.ilike.%$searchQuery%');
      }
      ...
      final dataResponse = await query
          .range(startIndex, startIndex + SystemConstants.itemsPorPagina - 1);

      return PaginatedResponse(items: dataResponse, totalCount: totalCount);
    } catch (e) {
